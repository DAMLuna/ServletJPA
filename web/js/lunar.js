...
function doEvent(kind, ms) {
    pause = true;
    var d = new Date();
    var fi= di.getDate() + '/' + (di.getMonth() + 1) + '/' + di.getFullYear() + ' ' + di.getHours() + ':' + di.getMinutes() + ':' + di.getSeconds()
    var fd = d.getDate() + '/' + (d.getMonth() + 1) + '/' + d.getFullYear() + ' ' + d.getHours() + ':' + d.getMinutes() + ':' + d.getSeconds();
    $('#state h1').addClass(kind);
    $('#state h1').html((kind == 'win') ? 'YOU WIN!' : 'YOU LOSE');
    $('#state h2').html((kind == 'win') ? 'Speed: ' + fd + 'm/s' : 'Maybe next time...');
    $('#state form').html((kind == 'win') ? '<input type="submit" formaction="/3.4.3ServletJPA/Game" value="Play Again"> <br> <input type="submit" formaction="index.jsp" value="Back">' : '<input type="submit" onclick="reset();" value="Play Again"><br><input type="submit" formaction="index.jsp" value="Back">');
    ...
    $(document).ready(function () {
        $.ajax({
            url: '/3.4.3ServletJPA/ServletStart',
            type: 'post',
            data: {
                fechaIni:fi,
                fechaFin: fd,
